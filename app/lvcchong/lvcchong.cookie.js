 ...
 * 驴充充 Cookie 获取脚本
 *
 * 抓取方式: 杀掉驴充充 App 重新冷启动(触发 /accessToken/refresh 拿 refreshToken)
 *   → 再进「我的 → 积分中心 / 签到」页(触发 /h5/accessEntrance 拿 phone + userId)
 *   两个请求都抓到才齐活;抓完关掉 App,避免 refreshToken 被 App 滚到新值。
 *
...
const $ = new Env("驴充充 [Cookie]");

const CK_AUTH = "lvcchong_auth"; // BoxJS 存的凭证 JSON
...
(function main() {
    ...
    try {
        ...
        if (/accessToken\/refresh/.test(url)) {
            // refresh 请求带:refreshToken(body)+ device_* 头 + 当前 userToken(token 头)
